...
- **大文件落盘**：每次 `write_file(content=...)` 控制在 {MAX_FILE_TOOL_CONTENT_CHARS} 字符以内；生成较长 HTML/CSS/JS/JSON manifest/base64/模板/文件正文时，不要把全文塞进一个 `write_file`。用 `execute_code` 创建/截断目标文件，再分块 append，最后用 `read_file` 或渲染检查校验。
