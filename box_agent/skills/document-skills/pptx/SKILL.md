...
4. call `generate_image` for every `generate` item before writing final slide HTML. Always pass `watermark: false` for PPT image generation — slides carry their own branding/watermark and the prompt already steers the model away from in-image watermarks, so the tool's default "AI Generated" stamp must be suppressed.
