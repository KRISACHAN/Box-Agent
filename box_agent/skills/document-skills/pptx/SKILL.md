...
Then synchronize once with one standalone Bash-tool command. The Bash tool
already runs with `BOX_AGENT_OUTPUT_DIR` as its cwd, so do not prepend `cd` and
do not spell the manifest as `$BOX_AGENT_OUTPUT_DIR/...`. Copy the exact
...
- macOS/Linux POSIX shells and the bundled Git Bash on Windows:
  `"$BOX_AGENT_NODE" "<LOADER_EXPANDED_SYNC_SCRIPT>" assets/generated/manifest.json`
- Windows PowerShell fallback when the Bash tool identifies itself as
  PowerShell:
  `& "$env:BOX_AGENT_NODE" "<LOADER_EXPANDED_SYNC_SCRIPT>" "assets/generated/manifest.json"`

The manifest argument above is the same literal artifact-relative path on every
...
the only command: do
not add `2>&1`, `echo`, `;`, `&&`, pipes, redirects, command substitutions,
...
4. **Topic needs facts you don't have?** If the deck needs evidence, market/
   ...
   `validate_outline.js` with that handoff's artifact-relative path passed to
   `--research-handoff`.
...
| Sync generated image statuses | Use the single platform-specific standalone command from §0 with the loader-expanded absolute script path; do not add `cd`, redirects, or diagnostic suffixes |
