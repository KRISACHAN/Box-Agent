...
7. `finalize_controlled_deck.js` is the only normal finalization command. Do not hand-edit the compiled HTML, split finalization into separate successful validator/render calls, or add another `output/` prefix. The generated HTML is the default deliverable and includes the controlled editor runtime. A non-zero finalizer exit or a failed structural, media, HTML self-check, editor-fit, contrast, or export-geometry report blocks delivery: fix the deck props or layout and rerun the finalizer. A source/truth advisory with findings does not block or invalidate an existing `index.html`; keep the HTML and summarize those findings afterward.
...
These are the blocking structural, media, HTML, and runtime gates.
...
If a blocking structural, media, HTML, runtime, or explicitly required export
step is blocked, write `BLOCKED` for that step. Never use `BLOCKED` for a
