...
**未获得用户模式选择时，下一步必须实际调用 `request_user_decision`，然后等待回复。**
...
1. 只有用户在本轮 prompt 中明确表示选择“快速模式”或“创意模式”，才能分别直接走
   `fast` 或 `creative`。提问、比较、引用或否定模式名称不等于作出选择；同时提到两个
   名称但未选定一个，也应弹出选择卡。
2. 同一任务中，用户之前明确选择过上述某一模式，或通过选择卡返回了
   `[HOST_USER_DECISION_RESPONSE]` 中 `decision_kind="presentation_mode"` 的
   `selected_option_id` 时，沿用其选择；本轮明确指定的新选择优先。仅重新加载 Skill、
...
3. 除上述明确选择外，一律调用下面的 `request_user_decision`。不要根据“自由设计、
   无模板、套用模板、静态、动态、动画、SN、Dazzle”、文件格式、上传的 PPTX、制作
...
仅在用户已选择创意模式后，才确定其内部的静态或动态出口。默认制作静态页面：
