...
def _rename_subset_font(target: Path, delivery_family: str, weight: str, style: str) -> None:
    ...
    font = TTFont(target, lazy=False, recalcTimestamp=False)
    try:
...
def _read_inputs(root: Path, from_plans: bool = False) -> tuple[str, list[str]]:
    ...
    plans = sorted(path for path in (root / "plan").glob("slide_*.md") if ".bak." not in path.name)
    slides = sorted(path for path in (root / "slides").glob("slide_*.html") if ".bak." not in path.name)
    if not (plans if from_plans else slides):
    ...
    # Both stages use the same current inputs. Planned screen copy anticipates
    # HTML creation; actual HTML still contributes any newly added glyphs/fonts.
    plans = [path for path in (root / "plan/design-brief.md", root / "plan/deck.md") if path.is_file()] + plans
    fragments = [_plan_font_fragment(path) for path in plans]
    fragments += [path.read_text(encoding="utf-8") for path in slides]
...
def bundle_workspace(root: Path, from_plans: bool = False) -> dict:
    ...
    _atomic_write(
        root / "base.css",
        bundle_fonts(root, css, fragments),
    )
