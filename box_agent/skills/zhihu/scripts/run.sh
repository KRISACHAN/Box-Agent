...
HOST_CLI_HOME=${ZHIHU_CLI_HOME:-}
FALLBACK_CLI_HOME="$HOME/Library/Application Support/zhihu-cli"
CLI_HOME=${HOST_CLI_HOME:-"$FALLBACK_CLI_HOME"}
...
if [ "${1:-}" = "setup" ]; then
  shift
  exec env -u ZHIHU_CLI_HOME "$SCRIPT_DIR/setup.sh" "$@"
fi
