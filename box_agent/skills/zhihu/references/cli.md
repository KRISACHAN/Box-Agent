...
返回 `installed=false` 或 `CLI_NOT_INSTALLED` 时，先取得用户明确同意，再运行同平台的 `setup.sh` 或 `setup.ps1`。setup 沿用知乎官方原始 Skill 流程，下载固定官方资源并校验 host、大小、SHA-256、归档结构和二进制版本，然后安装到用户目录。
