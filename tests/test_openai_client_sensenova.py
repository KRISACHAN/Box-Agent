...
def test_env_rejects_broad_or_unbounded_prefixes(monkeypatch):
    monkeypatch.setenv(
        "BOX_AGENT_SENSENOVA_MODEL_PREFIXES",
        "g, ab, broad, -, valid-family-, valid_family_",
    )

    prefixes = _sensenova_model_prefixes()

    assert "g" not in prefixes
    assert "ab" not in prefixes
    assert "broad" not in prefixes
    assert "-" not in prefixes
    assert "valid-family-" in prefixes
    assert "valid_family_" in prefixes
