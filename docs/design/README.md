...
| `agent_session.py`, `agent_run.py`, ACP/CLI session configuration and event streams | [Agent Session](../AGENT_SESSION.md), then [Layered architecture](../ARCHITECTURE.md) / [分层架构](../ARCHITECTURE_CN.md) | AgentSession owns live state and retains the session Config reference; ACP/CLI prepare host capabilities and render shared events. Legacy Agent APIs remain available, and Session Log remains the only durable state source. | `tests/test_agent_session.py`, `tests/test_cli_runtime.py`, `tests/test_cli_session_trace.py`, `tests/test_architecture_boundaries.py`, and ACP regressions |
