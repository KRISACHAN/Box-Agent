...
决策遵循“安全主路径优先推进”：如果模型能在不改变用户可见结果的前提下自行选择，
就不应弹卡；如果确需展示选择，但其中一个选项能安全延续用户的明确请求，应将其设为
默认项并申请 15-30 秒超时。只有没有明确安全默认项或涉及敏感决策时才保持纯人工选择。
“技术上可执行”本身不足以自动提交，默认路径还必须低风险、可回退并保持用户意图。
...
是保证。Skill 没有声明超时也不妨碍模型按上述通用原则为安全主路径补齐默认项和超时
申请。协议 v1 不要求 Skill manifest 声明能力；未实现卡片的宿主保留人工降级路径。
