...
未指定来源的 Skill 查找或安装请求默认包含来源发现：可以先查询市场，未命中或
不可用时继续搜索 GitHub 等公开来源并核对仓库和 `SKILL.md`；只有用户明确限定
仅从市场安装时才限制来源。市场查询可使用 `explicit_marketplace_request`，
该协议值不表示用户限定了来源。外部来源不能转换为虚构的市场 `skill_id`，
外部安装仍遵守权限规则，不能绕过已拒绝的安装授权。
